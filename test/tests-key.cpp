...
TEST_CASE("Test writing key") {
  ...
  k.timeStamp(12345)
   ...
   .length(345)
   .userData(0x566F7961676572ULL)  // Voyager
	 .version(0);

  const size_t len = setKey(k, tmp.data(), tmp.capacity());
  REQUIRE(39 == len);

  const unsigned char output[] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x30, 0x39, 0x00, 0x00, 0x10, 0xe1, 0x00, 0x03, 0x68, 0x70, 0x00, 0x00, 0x00, 0x00, 0x3a, 0xde, 0x68, 0xb1, 0x48, 0xac, 0xe3, 0x36, 0x01, 0x59, 0x00, 0x56, 0x6f, 0x79, 0x61, 0x67, 0x65, 0x72, 0x00 };
...
TEST_CASE("Test reading key") {
  ...
  const unsigned char input[] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x30, 0x39, 0x00, 0x00, 0x10, 0xe1, 0x00, 0x03, 0x68, 0x70, 0x00, 0x00, 0x00, 0x00, 0x3a, 0xde, 0x68, 0xb1, 0x48, 0xac, 0xe3, 0x36, 0x01, 0x59, 0x00, 0x56, 0x6f, 0x79, 0x61, 0x67, 0x65, 0x72, 0x00 };
  const size_t length{39};
  ...
  REQUIRE(0x566F7961676572 == k.userData());
  REQUIRE(0 == k.version());
}
